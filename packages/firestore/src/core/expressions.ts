...
export class CoreIsError implements EvaluableExpr {
  constructor(private expr: FunctionExpr) {}
  ...
  evaluate(
    ...
    hardAssert(
      this.expr.params.length === 1,
      'is_error() function should have exactly 1 param'
    );
    const evaluated = toEvaluable(this.expr.params[0]).evaluate(context, input);
    switch (evaluated.type) {
      case 'ERROR':
...
function adjustTimestamp(
  ...
  if (nanos < 0) {
    ...
    // Although the modulo operation should guarantee this range for negative results.
    return {
      seconds: seconds - 1,
...
export class CoreUnixMicrosToTimestamp extends UnixToTimestamp {
  ...
  toTimestamp(value: bigint): EvaluateResult {
    ...
    // Note: BigInt modulo can result in negative values if the dividend is negative.
    let nanos = Number((value % MICROSECONDS_PER_SECOND) * BigInt(1000));
...
export class CoreUnixMillisToTimestamp extends UnixToTimestamp {
  ...
  toTimestamp(value: bigint): EvaluateResult {
    ...
    let nanos = Number((value % MILLISECONDS_PER_SECOND) * BigInt(1000 * 1000));
